...
public class QFileClient {
  ...
  private final File localScratchDirectory;
  ...
  public QFileClient(HiveConf hiveConf, String hiveRootDirectory, String qFileDirectory, String outputDirectory,
    ...
    this.localScratchDirectory = new File(hiveConf.getVar(ConfVars.LOCALSCRATCHDIR));
  ...
  void initFilterSet() {
    ...
    .addFilter(localScratchDirectory.toString() + "[\\w\\-/]+", "!!{hive.exec.local.scratchdir}!!")
    ...
    .addFilter(userName + "\\s*", "!!{user.name}!!")
    .addFilter(operatorPattern, "\"$1_!!ELIDED!!\"")

     //mask stats for now, as it is not being reliably reported
    .addFilter("totalSize=\\d+", "!!{totalSize}!!")
    .addFilter("numRows=\\d+", "!!{numRows}!!")
    .addFilter("rawDataSize=\\d+", "!!{rawDataSize}!!")
